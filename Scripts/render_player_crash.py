...
if args.recovery:
 ...
 lifecycle=re.findall(r'CrashLifecycle: (\{[^\n]+\})',text);r['lifecycle']=json.loads(lifecycle[-1]) if lifecycle else None
 r['transfer']=json.loads(transfer[0]) if transfer else None;r['recovery']=json.loads(complete[-1]) if complete else None
...
print(json.dumps(r));raise SystemExit(0 if r['capture_passed'] and r['skin_clearance_passed'] and r['physics']['passed'] and (not args.recovery or ((r.get('recovery') or {}).get('passed') and (r.get('lifecycle') or {}).get('passed'))) and (not args.bike or (r.get('bike') or {}).get('passed')) else 1)
