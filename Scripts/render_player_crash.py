...
parser=argparse.ArgumentParser();parser.add_argument('--recovery',action='store_true');parser.add_argument('--bike',action='store_true');parser.add_argument('--double-road',action='store_true');parser.add_argument('--solid-support',action='store_true');args=parser.parse_args()
out=root/'work/player-crash-runtime'/uuid.uuid4().hex
out.mkdir(parents=True)
with (out/'run.log').open('w') as log:
 result=subprocess.run(['/Volumes/Adam Assets/Unreal/UE_5.8/Engine/Binaries/Mac/UnrealEditor-Cmd',str(root/'AuraPlayground.uproject'),'/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty=Easy?AutoStart=1','-game','-RenderOffscreen','-RCWebControlDisable','-BattleSkipTutorial','-BattlePlayerCrashReview',*(['-BattlePlayerRecoveryBlend'] if args.recovery else []),*(['-BattleFallenBikeReview'] if args.bike else []),*(['-BattleCrashDoubleRoad'] if args.double_road else []),*(['-BattleCrashSolidSupport'] if args.solid_support else []),f'-BattleHUDReviewDir={out}','-windowed','-ResX=1280','-ResY=720','-ForceRes','-unattended','-nosound','-ExecCmds=r.ScreenshotDelegate 0,t.IdleWhenNotForeground 0','-stdout'],stdout=log,stderr=subprocess.STDOUT,timeout=180)
...
report='2026-09-12-player-solid-support.json' if args.solid_support else '2026-09-12-player-bike-double-road.json' if args.double_road else '2026-09-12-player-bike-fall.json' if args.bike else '2026-09-12-player-fall-recovery.json' if args.recovery else '2026-09-12-player-crash-runtime.json'
