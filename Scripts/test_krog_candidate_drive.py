...
parser=argparse.ArgumentParser();parser.add_argument('--report',default='2026-09-12-krog-candidate-drive.json');parser.add_argument('--level-floor',action='store_true');parser.add_argument('--continuous-shell',action='store_true');parser.add_argument('--buildings',action='store_true');parser.add_argument('--finish-route',action='store_true');parser.add_argument('--rail',action='store_true');parser.add_argument('--world',action='store_true');parser.add_argument("--crowds",action="store_true");parser.add_argument("--main",action="store_true");parser.add_argument("--real-handling",action="store_true");args=parser.parse_args()
assert sum([args.level_floor,args.continuous_shell,args.buildings,args.rail,args.world,args.main])<=1
assert Path(args.report).name==args.report
app=Path('/Volumes/Adam Assets/Unreal/UE_5.8/Engine/Binaries/Mac/UnrealEditor-Cmd')
variant = 'main' if args.main else 'world' if args.world else 'rail' if args.rail else 'buildings' if args.buildings else 'continuous-shell' if args.continuous_shell else 'level-floor' if args.level_floor else 'candidate'
route = 'finish-drive' if args.finish_route else 'drive'
log = root / f'work/krog-{variant}-{route}.log'
map_name = ('PiedmontWorld' if args.main else 'PiedmontKrogWorldReview' if args.world else 'PiedmontKrogRailReview' if args.rail else 'PiedmontKrogBuildingsReview' if args.buildings else
...
with log.open('w') as stream:
    run=subprocess.run([str(app),str(root/'AuraPlayground.uproject'),f'/Game/PiedmontRide/Maps/{map_name}?Difficulty=Easy?AutoStart=1','-game','-BattleSkipTutorial','-RCWebControlDisable',
        '-nullrhi','-unattended','-nosound','-BattleHomeDriveAudit' if args.finish_route else '-BattleKrogAudit','-stdout']+(['-BattleKeepCrowds'] if args.crowds else [])+(['-BattleRealHandlingRoute'] if args.real_handling else []),stdout=stream,stderr=subprocess.STDOUT,timeout=600)
...
if args.world or args.main:result['scope']='Combined map: W/A/D ride on selected route with road traffic directors active. Pedestrian traffic disabled by existing route fixture; not a full mixed-crowd playthrough.'
if args.crowds:
...
if args.real_handling:
    result['passed']=bool(result['passed'] and result['handling'] and result['handling']['realistic'])
    result['scope']+=' Realistic handling uses test-driver Space braking before mapped bends and nearby cars; this is not player autopilot or complete handling acceptance.'
