"""Build opposing Irwin/Lake routes using source road and installed trail triangles."""
import json,math,collections
from pathlib import Path
from shapely.geometry import LineString
from shapely.ops import linemerge
root=Path(__file__).resolve().parents[1];out=root/'SourceAssets/Terrain/IrwinTraffic'
network=json.loads((out/'network.json').read_text())
line=linemerge([LineString([p[:2] for p in row['points_cm']]) for row in network['roads'] if row['tags']['name'] in ['Irwin Street Northeast','Lake Avenue Northeast']]);assert line.geom_type=='LineString'
if line.coords[0][0]>line.coords[-1][0]:line=LineString(list(line.coords)[::-1])
triangles=[];grid=collections.defaultdict(list)
meshes=[(out/'Irwin_Road.obj',-1)]
...
for name,offset,reverse in [('eastbound',150,False),('westbound',-150,True)]:
 points=[]
 ...
 routes.append({'name':name,'points_cm':points,'speed_cm_s':500,'scope':'Irwin/Lake candidate; native support, crossing controls and endpoint visibility pending.'})
report={'author':'2026-09-12 [codex-maclaptop]','source':'OpenStreetMap contributors; retained Irwin/Lake network and native-reviewed surfaces','road_length_cm':line.length,'source_coverage_passed':not failures,'failures':failures,'routes':routes,'main_map_changed':False}
