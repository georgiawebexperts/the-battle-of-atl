"""Build flush roadside pavement against repaired roads and existing trail."""
...
road = unary_union([LineString(r['points_cm']).buffer(480, join_style=2)
...
# Include the repaired road as a preserved adjoining surface.
vertices=[]
for line in (folder/'Irwin_Road.obj').read_text().splitlines():
    fields=line.split()
    if fields and fields[0]=='v':vertices.append((float(fields[1]),-float(fields[2]),float(fields[3])))
        ...
        poly=Polygon([p[:2] for p in face])
        if poly.area>1e-5:polys.append(poly);triangles.append(face)
assert polys
index = STRtree(polys); protected = unary_union(polys)
buildings=json.loads((root/'SourceAssets/Terrain/IrwinBuildings/buildings.json').read_text())['buildings']
building_footprints=unary_union([Polygon(b['footprint_xy']) for b in buildings])
geometry = road.difference(protected).difference(building_footprints)
if geometry.geom_type=='GeometryCollection':
    geometry=unary_union([p for p in geometry.geoms if p.geom_type in ['Polygon','MultiPolygon']])
...
# Report sub-millimetre remnants from rounded polygon clipping explicitly.
max_open_edge=max((math.dist(a,b) for a,b in interior_open_edges),default=0)
assert max_open_edge<.1,interior_open_edges[:5]
...
lines=['o Irwin_Sidewalk']+[f'v {x:.5f} {-y:.5f} {z:.5f}' for x,y,z in vertices]
...
(folder/'Irwin_Sidewalk.obj').write_text('\n'.join(lines)+'\n')
...
(folder/'sidewalk-probes.json').write_text(json.dumps({'samples':probes},indent=2)+'\n')
report={'conformed_edges':split_edges,'max_previous_edge_height_gap_cm':max_previous_gap,'interior_open_edges':len(interior_open_edges),'max_open_edge_cm':max_open_edge,'triangles':len(faces),'missing_area_cm2':missing,'protected_overlap_cm2':coverage.intersection(protected).area,
        'source_to_native_max_error_cm':max(errors),'native_reference_samples':len(errors),
        'main_map_changed':False,'scope':'Flush roadside pavement candidate; native support, edge transitions and appearance pending. Door landings not included.'}
(folder/'sidewalk-surfaces.json').write_text(json.dumps(report,indent=2)+'\n');print(report)
