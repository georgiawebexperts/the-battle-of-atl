...
mesh.set_material(0,unreal.load_asset('/Game/PiedmontRide/Materials/M_ParkConcreteWorld'));mesh.get_editor_property('body_setup').set_editor_property('collision_trace_flag',unreal.CollisionTraceFlag.CTF_USE_COMPLEX_AS_SIMPLE);unreal.EditorAssetLibrary.save_loaded_asset(mesh)
...
(root/'Tests/Results/2026-09-13-krog-approach-review.json').write_text(json.dumps({'main_map_changed':False,'offset_cm':220,'width_cm':240,'surface_samples':len(indices),'surface_failures':failures,'triangles':len(faces),'scope':'Authored approach fitted to native ground in separate map; no visual, traffic or navigation acceptance yet.'},indent=2)+'\n')
