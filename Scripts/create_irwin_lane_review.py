"""Check car envelope support and create isolated Irwin/Lake drive review."""
import json
from pathlib import Path
import unreal
root=Path(unreal.Paths.project_dir());folder=root/'SourceAssets/Terrain/IrwinTraffic'
assert unreal.EditorLoadingAndSavingUtils.load_map('/Game/PiedmontRide/Maps/PiedmontIrwinRoadReview')
...
if not failures:
 for row in source['routes']:
  ...
  car.set_actor_label('Irwin Lake '+row['name']);car.tags=list(car.tags)+[unreal.Name('IrwinCarLaneReview'),unreal.Name('MonroeCarLaneReview')]
 ...
 assert unreal.EditorLoadingAndSavingUtils.save_map(unreal.get_editor_subsystem(unreal.UnrealEditorSubsystem).get_editor_world(),'/Game/PiedmontRide/Maps/PiedmontIrwinLaneReview')
...
(root/'Tests/Results/2026-09-12-irwin-lane-support.json').write_text(json.dumps(report,indent=2)+'\n')
