...
parser=argparse.ArgumentParser();parser.add_argument('--packaged',action='store_true');parser.add_argument('--serial-pso',action='store_true');parser.add_argument('--close',action='store_true');parser.add_argument('--right-side',action='store_true');parser.add_argument('--markers',action='store_true');parser.add_argument('--full-shaders',action='store_true');parser.add_argument('--weapon',choices=['pistol','shotgun','smg','rifle'],default='pistol');parser.add_argument('--report',default='2026-09-13-third-person-aim-pitch.json');args=parser.parse_args();assert Path(args.report).name==args.report
...
support=re.findall(r'BattleAimSupportTravel: ([-0-9.]+)',(out/'run.log').read_text())
r['vertical_support_hand_travel_cm']=float(support[-1]) if support else None
policy=re.findall(r'BattlePipelinePolicy: async=(\d+)',(out/'run.log').read_text())
r.update(async_pipeline_compile=int(policy[-1]) if policy else None,serial_pso_diagnostic=args.serial_pso,weapon=args.weapon,right_side=args.right_side,full_shaders=args.full_shaders,markers=args.markers,close_view=args.close,max_weapon_origin_to_hand_cm=float(distance[-1]) if distance else None,exit_code=run.returncode,images=[str(capture/f'aim-{i}.png') for i in [1,2,3]],log=str(out/'run.log'),visual_review=False,packaged=args.packaged)
