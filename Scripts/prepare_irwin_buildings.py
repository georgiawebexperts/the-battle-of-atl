...
road=unary_union([LineString(r['points_cm']).buffer(330,join_style=2) for r in json.loads((root/'SourceAssets/Terrain/IrwinTraffic/network.json').read_text())['roads']])
...
for e in json.loads((root/'References/irwin-lake-buildings-osm.json').read_text())['elements']:
 ...
 clipped=poly.difference(road);assert clipped.geom_type=='Polygon' and not clipped.interiors
 coords=list(clipped.exterior.coords)[:-1];heights=[height(x,y) for x,y in coords]
 rows.append({'osm_way':e['id'],'tags':e['tags'],'original_footprint_xy':list(poly.exterior.coords),'footprint_xy':coords,'road_trim_area_cm2':overlap,'base_z_cm':max(heights)+10,'foundation_z_cm':min(heights)-30,'storeys':ids[e['id']],'storey_height_cm':340,'height_source':'Gameplay estimate, not surveyed height','terrain_range_cm':[min(heights),max(heights)]})
assert len(rows)==len(ids)
r={'author':'2026-09-12 [codex-maclaptop]','source':'OpenStreetMap contributors ODbL; References/irwin-lake-buildings-osm.json','frame':'world ESU centimetres, one-third footprint geography; full gameplay storey heights','buildings':rows,'main_map_changed':False}
