"""Mark Irwin/Lake lanes while keeping the continuous trail crossing clear."""
import json,math
...
root=Path(__file__).resolve().parents[1];folder=root/'SourceAssets/Terrain/IrwinTraffic'
network=json.loads((folder/'network.json').read_text());white=[];yellow=[]
clear=unary_union([LineString(network['trail_crossing']['points_cm']).buffer(400),Point(28099.55731,100846.51825).buffer(500)])
for name in ['Irwin Street Northeast','Lake Avenue Northeast','Krog Street Northeast']:
 ...
 for path in ([merged] if merged.geom_type=='LineString' else merged.geoms):
  ...
  for offset in [-280,280]:white.append(path.offset_curve(offset,join_style=2).buffer(5,cap_style=2))
...
for name,paint in paints.items():
 ...
 lines=['o Irwin_'+name]+[f'v {x:.4f} {-y:.4f} {z:.4f}' for x,y,z in vertices]+[f'vt {x/200:.5f} {y/200:.5f}' for x,y,z in vertices]
 for i in range(0,len(vertices),3):lines.append('f '+' '.join(f'{j}/{j}' for j in [i+3,i+2,i+1]))
 (folder/f'Irwin_{name}.obj').write_text('\n'.join(lines)+'\n');results.append({'file':f'Irwin_{name}.obj','triangles':len(faces)})
(folder/'markings.json').write_text(json.dumps({'surfaces':results,'scope':'Double yellow centre lines and white road edges, cleared at trail and Krog junction. No zebra stripes or signals inferred at the source-unmarked crossing; visual review pending.','clearance_cm':1},indent=2)+'\n');print(results)
