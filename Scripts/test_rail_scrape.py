...
for real in [False,True]:
 log=root/f'work/rail-scrape-{uuid.uuid4().hex}.log'
 with log.open('w') as f:
  run=subprocess.run(entry+['/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty=Easy?AutoStart=1','-game','-nullrhi','-RCWebControlDisable','-BattleSkipTutorial','-BattleSteeringAudit','-BattleRailScrapeAudit','-unattended','-nosound','-stdout']+(['-BattleRealHandlingAudit'] if real else []),stdout=f,stderr=subprocess.STDOUT,timeout=90)
 matches=re.findall(r'RailScrapeAudit: (\{[^\n]+\})',log.read_text())
...
report={'passed':all(r['passed'] for r in results),'packaged':a.packaged,'runs':results,'scope':'Actual forward/reverse input alongside a solid rail. Does not prove every bridge turn.'}
