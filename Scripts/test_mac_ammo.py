...
with log.open('w') as stream:
 run=subprocess.run(entry+[f'/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty={a.difficulty}?AutoStart=1','-game','-RCWebControlDisable','-unattended','-nosound','-BattleSkipTutorial','-BattleAmmoAudit','-stdout']+extra,stdout=stream,stderr=subprocess.STDOUT,timeout=120)
...
result.update(bin_layout=a.bins,images=[str(capture/'ammo-bin.png')] if a.review and a.bins else [])
