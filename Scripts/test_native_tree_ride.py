...
p=argparse.ArgumentParser();p.add_argument('--real',action='store_true');p.add_argument('--app',type=pathlib.Path);p.add_argument('--report',default='2026-09-13-native-tree-knockoffs.json');args=p.parse_args();assert pathlib.Path(args.report).name==args.report
...
with log.open('w') as stream:
 run=subprocess.run(([str(args.app)] if args.app else [engine,str(root/'AuraPlayground.uproject')])+(['-BattleRealTreeAudit'] if args.real else [])+['/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty=Easy?AutoStart=1','-game','-nullrhi','-BattleSkipTutorial','-BattleTreeRideAudit','-unattended','-nosound','-stdout'],stdout=stream,stderr=subprocess.STDOUT,timeout=180)
rows=re.findall(r'TreeRideAudit: (\{[^\n]+\})',log.read_text());r={'exit_code':run.returncode,'checks':json.loads(rows[-1]) if rows else None,'scope':'Native keyboard passes beside and into three authored tree instances, one per mesh type. Nearby pedestrians/zombies removed; does not establish full-path clearance, realistic crash animation, on-foot navigation or packaged performance.'};r['real_handling']=args.real;r['packaged']=bool(args.app);r['passed']=run.returncode==0 and r['checks'] is not None and r['checks']['passed']
