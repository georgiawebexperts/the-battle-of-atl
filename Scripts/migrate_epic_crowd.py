...
report=target.parent/'work/epic-crowd-migration.json'
roots=json.loads((target.parent/'SourceAssets/Manifests/epic-crowd-selection.json').read_text())['roots']
r=unreal.AssetRegistryHelpers.get_asset_registry();r.search_all_assets(synchronous_search=True)
options=unreal.AssetRegistryDependencyOptions()
hard_options=unreal.AssetRegistryDependencyOptions(include_soft_package_references=False)
required=set(roots);missing_soft=[]
...
while queue:
 ...
 visited.add(package)
 required.update(str(p) for p in r.get_dependencies(package,hard_options))
...
for package in sorted(visited):
 ...
 if not found:
  if package in required:raise RuntimeError('Missing required dependency package: '+package)
  missing_soft.append(package)
for row in files:
...
report.write_text(json.dumps({'roots':roots,'packages':sorted(visited),'external_dependencies':sorted(external),'missing_optional_references':missing_soft,'files':files,'total_bytes':sum(f['bytes'] for f in files),'status':'copied; native render acceptance pending'},indent=2)+'\n')
print('BATTLE_CROWD_MIGRATION_COMPLETE',len(files))
