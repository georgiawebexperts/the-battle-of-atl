...
assert not (level_floor and continuous_shell)
suffix='continuous-shell' if continuous_shell else ('level-floor' if level_floor else 'road')
...
map_name='PiedmontKrogShellReview' if continuous_shell else ('PiedmontKrogFloorReview' if level_floor else 'PiedmontKrogRoadReview')
...
(root/'Tests/Results'/f'2026-09-12-krog-{suffix}-render.json').write_text(json.dumps({'images':images,'main_map_changed':False,'headroom_samples':clearance,'minimum_headroom_cm':min(p['headroom_cm'] for p in clearance),'visual_review':'pending'},indent=2)+'\n')
