"""Fill narrow road-segment joins without paving over the protected cycle track."""
...
rows=json.loads((folder/'network.json').read_text())['roads'];rows.sort(key=lambda r:min(p[0] for p in r['points_cm']))
patches=[]
for first,second in zip(rows,rows[1:]):
 a=first['points_cm'];b=second['points_cm'];a=a if a[0][0]<a[-1][0] else a[::-1];b=b if b[0][0]<b[-1][0] else b[::-1]
 assert math.dist(a[-1][:2],b[0][:2])<1
 site=a[-1];width=min(int(first['tags']['lanes']),int(second['tags']['lanes']))*300;corners=[]
 for p,q in [(a[-2],a[-1]),(b[0],b[1])]:
  dx,dy=q[0]-p[0],q[1]-p[1];l=math.hypot(dx,dy);dx/=l;dy/=l
  for along in [-20,20]:
   for side in [0,width]:corners.append((site[0]+dx*along-dy*side,site[1]+dy*along+dx*side))
...
(folder/'TenthStreet_RoadSeams.obj').write_text('\n'.join(lines)+'\n')
(folder/'road-seams.json').write_text(json.dumps({'author':'2026-09-12 [codex-maclaptop]','area_cm2':patch.area,'triangles':len(faces),'protected_cycle_overlap_cm2':patch.intersection(protected).area,'installed':False},indent=2)+'\n');print(patch.area,len(faces))
