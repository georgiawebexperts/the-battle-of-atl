...
pothole=json.loads((folder/'pothole-site.json').read_text())
...
def height(x,y):
    ...
    dx,dy=x-pothole['xyz'][0],y-pothole['xyz'][1];angle=math.atan2(dy,dx)
    radius=pothole['radius_cm']*(1+.08*math.sin(5*angle)+.05*math.cos(7*angle))
    r=math.hypot(dx,dy)/radius
    depression=pothole['depth_cm']*max(0,1-r*r)**2
    return terrain(x,y)+14 + blend*(plane(i,q.x,q.y)-terrain(q.x,q.y)-14)-depression
...
lines=['o Irwin_PotholeRoad']+[f'v {x:.5f} {-y:.5f} {z:.5f}' for x,y,z in vertices]
...
(folder/'Irwin_PotholeRoad.obj').write_text('\n'.join(lines)+'\n')
...
(folder/'pothole-road-probes.json').write_text(json.dumps({'samples':probes},indent=2)+'\n')
...
(folder/'pothole-road-surfaces.json').write_text(json.dumps(report,indent=2)+'\n');print(report)
