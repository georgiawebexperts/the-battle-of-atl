...
log=root/f'work/build036-drone-{a.difficulty.lower()}.log'
with log.open('w') as stream:
 run=subprocess.run([str(app),f'/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty={a.difficulty}?AutoStart=1','-nullrhi','-unattended','-nosound','-BattleDroneAudit','-stdout'],stdout=stream,stderr=subprocess.STDOUT,timeout=90)
...
(root/f'Tests/Results/2026-09-11-build036-drone-{a.difficulty.lower()}.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
