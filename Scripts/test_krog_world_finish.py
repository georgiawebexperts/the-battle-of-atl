...
parser=argparse.ArgumentParser();parser.add_argument('--difficulty',choices=['Easy','Hard'],default='Easy');parser.add_argument('--render',action='store_true');parser.add_argument('--celebration',action='store_true');args=parser.parse_args();args.render=args.render or args.celebration
...
command=[engine,str(root/'AuraPlayground.uproject'),f'/Game/PiedmontRide/Maps/PiedmontKrogWorldReview?Difficulty={args.difficulty}?AutoStart=1','-game','-BattleSkipTutorial','-BattleFinishAudit','-RCWebControlDisable','-unattended','-nosound','-stdout']
...
result.update(exit_code=r.returncode,difficulty=args.difficulty,map='PiedmontKrogWorldReview',scope='Native trigger/guard fixture using teleports for phone, checkpoints and ordered tunnel entry/exit. Easy mounted finish; Hard on-foot finish. Not a complete route playthrough. Audit-only save slot verified in BattleFinish.cpp.')
...
(root/f'Tests/Results/2026-09-12-krog-world-{"celebration" if args.celebration else "finish"}-{args.difficulty}.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result));raise SystemExit(0 if result['passed'] else 1)
