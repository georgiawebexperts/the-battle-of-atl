...
entry=[str(bundle/'Contents/MacOS/AuraPlayground')] if args.packaged else [engine,str(root/'AuraPlayground.uproject')]
capture=out
if args.packaged and args.render:
 bid=plistlib.loads((bundle/'Contents/Info.plist').read_bytes())['CFBundleIdentifier']
 capture=Path.home()/'Library/Containers'/bid/'Data/Documents/BattleFinishReview'/uuid.uuid4().hex;capture.mkdir(parents=True)
...
command+=['-RenderOffscreen','-BattleFinishReview',f'-BattleHUDReviewDir={capture}','-windowed','-ResX=1280','-ResY=720','-ForceRes','-ExecCmds=r.ScreenshotDelegate 0,t.IdleWhenNotForeground 0'] if args.render else ['-nullrhi']
