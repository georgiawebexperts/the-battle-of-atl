...
root=pathlib.Path(unreal.Paths.project_dir());out=root/'work/lake-path-lod-review';out.mkdir(parents=True,exist_ok=True)
...
images=[]
for name,position,target in [(n+'-'+label,p,t) for label in ['default','detail'] for n,p,t in views]:
 unreal.SystemLibrary.execute_console_command(world,'landscape.OverrideLOD0ScreenSize '+('0.01' if name.endswith('detail') else '-1'))
