...
p=argparse.ArgumentParser();p.add_argument('--punk',action='store_true');p.add_argument('--sharp',action='store_true');p.add_argument('--no-motion-blur',action='store_true');a=p.parse_args()
...
with log.open('w') as f:
 r=subprocess.run([str(app/'Contents/MacOS/AuraPlayground'),'/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty=Easy?AutoStart=1',*(['-ExecCmds=r.MotionBlurQuality 0,r.AntiAliasingMethod 1'] if a.sharp else ['-ExecCmds=r.MotionBlurQuality 0'] if a.no_motion_blur else []),'-BattleSkipTutorial','-BattleLocomotionReview','-BattleZombieReview',*(['-BattlePunkReview'] if a.punk else []),f'-BattleHUDReviewDir={folder}','-RenderOffscreen','-ResX=1280','-ResY=720','-ForceRes','-NoTextureStreaming','-unattended','-nosound','-stdout'],stdout=f,stderr=subprocess.STDOUT,timeout=90)
lines=[x for x in log.read_text().splitlines() if 'LocoReview:' in x]
out={'style':style,'app':str(app),'exit_code':r.returncode,'measurements':lines,'images':[str(folder/name) for name in ['walk.png','walk-step.png','run.png','idle.png']],'visual_review':'pending','scope':'Fixed walking/running/idle capture; fixture metrics include idle toe orientation, not all moving foot orientations.'}
out['passed']=r.returncode==0 and any('pass=1 ' in x for x in lines) and all(pathlib.Path(x).is_file() for x in out['images'])
(root/f'Tests/Results/2026-09-12-build052-{style}-motion.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out));raise SystemExit(0 if out['passed'] else 1)
