...
p=argparse.ArgumentParser();p.add_argument('--packaged',action='store_true');p.add_argument('--report',required=True);p.add_argument('--tunnel',action='store_true');args=p.parse_args();assert pathlib.Path(args.report).name==args.report
...
with (out/'run.log').open('w') as f:
 command=entry+['/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty=Easy?AutoStart=1','-game','-RenderOffscreen','-BattleSkipTutorial','-BattleHUDReview','-BattleObjectiveReview','-BattleDroneReview','-BattleDroneAimReview',f'-BattleHUDReviewDir={capture}','-RCWebControlDisable','-unattended','-nosound','-ResX=1280','-ResY=720','-windowed','-stdout']
 if args.tunnel:command=[v for v in command if v not in ['-BattleObjectiveReview','-BattleDroneReview','-BattleDroneAimReview']]+['-BattleTunnelExposureReview']
 r=subprocess.run(command,stdout=f,stderr=subprocess.STDOUT,timeout=120)
images=[]
