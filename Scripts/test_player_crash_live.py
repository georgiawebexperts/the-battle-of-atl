...
p=argparse.ArgumentParser();p.add_argument('--app',type=pathlib.Path);p.add_argument('--report');mode=p.add_mutually_exclusive_group();mode.add_argument('--death',action='store_true');mode.add_argument('--repeat',action='store_true');mode.add_argument('--death-on-foot',action='store_true');args=p.parse_args()
...
r['capture_passed']=len(r['images'])==(6 if args.repeat else 2 if args.death else 3)
...
(root/'Tests/Results'/(args.report or ('2026-09-12-player-crash-live-repeat.json' if args.repeat else '2026-09-12-player-crash-live-death-on-foot.json' if args.death_on_foot else '2026-09-12-player-crash-live-death.json' if args.death else '2026-09-12-player-crash-live.json'))).write_text(json.dumps(r,indent=2)+'\n')
