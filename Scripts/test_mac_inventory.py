...
with log.open('w') as f:
 run=subprocess.run([*entry,f'/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty={a.difficulty}?AutoStart=1',*flags,'-unattended','-nosound','-BattleInventoryAudit','-BattleSkipTutorial','-ExecCmds=t.IdleWhenNotForeground 0','-stdout'],stdout=f,stderr=subprocess.STDOUT,timeout=300 if a.full_shaders else 90)
...
result['exit_code']=run.returncode;result['passed']=bool(result.get('passed') and run.returncode==0);result['rendered_appearance_verified']=False;result['editor']=a.editor;result['full_shaders']=a.full_shaders;result['detailed_rider']='DetailedFootPreview: body=m_tal_nrw_body' in log.read_text();result['log']=str(log);result['images']=[str(x) for x in out.glob('*.png')]
