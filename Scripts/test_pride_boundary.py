...
p=argparse.ArgumentParser();p.add_argument("--app",type=Path);p.add_argument("--main",action="store_true");p.add_argument("--report",default="2026-09-13-pride-boundary.json");args=p.parse_args()
...
if args.app or args.main:cmd[2]=cmd[2].replace('PiedmontPrideStreetReview','PiedmontWorld')
...
text=log.read_text(errors='replace');matches=re.findall(r'BattleTutorialAudit: (\{[^\n]+\})',text);r=json.loads(matches[-1]) if matches else {'passed':False,'missing_report':True};r['passed']=r['passed'] and code==0;r.update(exit_code=code,packaged=bool(args.app),scope='Runtime fences, cleared old cutoff, market gate and coming-soon notice; not a full moving gate approach');(root/'Tests/Results'/args.report).write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r));raise SystemExit(0 if r['passed'] else 1)
