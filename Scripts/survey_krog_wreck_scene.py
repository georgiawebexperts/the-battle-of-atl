...
rows=[]
for back in (1100,1500,1900):
 for side in (-850,850):
  x,y=a[0]-dx*back-dy*side,a[1]-dy*back+dx*side
  points=[]
  for ox,oy in ((0,0),(-180,-140),(-180,140),(180,-140),(180,140)):
   h=unreal.PiedmontWorldTools.trace_world_surface(unreal.Vector(x+ox,y+oy,a[2]+500),unreal.Vector(x+ox,y+oy,a[2]-600))
...
r={'source':'Current main-world north Krog portal and native traces','candidates':rows,'preferred_candidate':{'back_from_portal_cm':1900,'lateral_offset_cm':850,'reason':'Approximately293cm conservative road clearance with about20cm footprint grade variation; native body/pose checks still required.'},'scope':'Ground survey only. Source-road-width bounding-circle clearance included; native lane/body clearance, pedestrian nav, pose footprint and rarity behavior not yet verified. No actors or map changes.'}
