...
assert json.loads((root/'Tests/Results/2026-09-12-native-monroe-lanes.json').read_text())['passed']
assert unreal.EditorLoadingAndSavingUtils.load_map('/Game/PiedmontRide/Maps/PiedmontMonroeTrafficReview')
...
assert unreal.EditorLoadingAndSavingUtils.save_map(world,'/Game/PiedmontRide/Maps/PiedmontMonroeCrossingReview')
(root/'Tests/Results/2026-09-12-monroe-crossing-placement.json').write_text(json.dumps({'bounds_xy':[lo,hi],'bindings':bindings,'main_map_changed':False,'signals_visible':False,'scope':'Accelerated 2/1/6 test cycle, shared exclusive reservation. Live traffic timing and visible poles pending.'},indent=2)+'\n')
