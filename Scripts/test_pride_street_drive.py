...
p=argparse.ArgumentParser();p.add_argument('--real',action='store_true');p.add_argument('--gate',action='store_true');p.add_argument('--review',action='store_true');p.add_argument('--lookahead',type=int,default=180);args=p.parse_args();root=Path(__file__).resolve().parents[1];mode=('real' if args.real else 'arcade')+('-rendered' if args.review else '')+f'-lookahead{args.lookahead}'+('-gate' if args.gate else '');log=root/f'work/pride-drive-{mode}.log';capture=root/'work/pride-drive-captures'/uuid.uuid4().hex
