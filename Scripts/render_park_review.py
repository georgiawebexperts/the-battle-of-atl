...
capture.set_editor_property('capture_every_frame',False);capture.set_editor_property('capture_on_movement',False);capture.set_editor_property('fov_angle',85)
...
for name,position,target in views:
 ...
 for _ in range(8):capture.capture_scene()
...
result={'created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'map':'/Game/PiedmontRide/Maps/PiedmontWorld','map_saved':False,'capture':'static editor SceneCapture2D, diagnostic light override','texture_streaming':0,'force_lod':unreal.SystemLibrary.get_console_variable_int_value('r.ForceLOD'),'lights':lights,'images':images,'scope':'Static scene only. No game-mode spawning, HUD, input, audio or FPS acceptance.'}
