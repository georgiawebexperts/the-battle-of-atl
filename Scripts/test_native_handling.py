...
for real in (False,True):
 ...
 log=root/f'work/handling-{name}-{uuid.uuid4().hex}.log'
 ...
 result.update(mode=name,exit_code=run.returncode,log=str(log));result['passed']=result['passed'] and run.returncode==0;results.append(result)
