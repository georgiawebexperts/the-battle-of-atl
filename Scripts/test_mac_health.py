"""Exercise shared health and ordered checkpoint recovery in the cooked Mac game."""
...
with log.open('w') as stream:
    run=subprocess.run([str(app),'/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty=Easy?AutoStart=1',
        '-nullrhi','-unattended','-nosound','-BattleHealthAudit','-stdout'],stdout=stream,stderr=subprocess.STDOUT,timeout=90)
matches=re.findall(r'BattleHealthAudit: (\{[^\n]+\})',log.read_text())
