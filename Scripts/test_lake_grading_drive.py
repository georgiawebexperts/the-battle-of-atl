...
p=argparse.ArgumentParser();p.add_argument('--real',action='store_true');p.add_argument('--path',default='61853018');p.add_argument('--lookahead',type=int);p.add_argument('--brake-bends',action='store_true');p.add_argument('--min-length',type=int,default=0);p.add_argument('--review',action='store_true');p.add_argument('--main',action='store_true');args=p.parse_args()
root=Path(__file__).resolve().parents[1];mode=('real' if args.real else 'arcade')+(f'-lookahead{args.lookahead}' if args.lookahead else '')+('-braked' if args.brake_bends else '')+(f'-min{args.min_length}' if args.min_length else '')+('-review' if args.review else '')+('-main' if args.main else '');log=root/f'work/lake-drive-{args.path}-{mode}.log'
...
text=log.read_text(errors='replace');rows=re.findall(r'BattleConnectorAudit: (\{[^\n]+\})',text);report=json.loads(rows[-1]) if rows else {'passed':False,'missing_report':True};report.update(exit_code=code,mode=mode,path=args.path,scope='Uncooked '+('main' if args.main else 'candidate')+' map, two-direction keyboard traversal. Not visual or packaged acceptance.');report['passed']=report['passed'] and code==0
