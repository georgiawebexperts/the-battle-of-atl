...
p=argparse.ArgumentParser();p.add_argument('--real',action='store_true');p.add_argument('--path',default='61853018');p.add_argument('--lookahead',type=int);p.add_argument('--brake-bends',action='store_true');p.add_argument('--min-length',type=int,default=0);p.add_argument('--review',action='store_true');p.add_argument('--main',action='store_true');p.add_argument('--app',type=Path);p.add_argument('--report');args=p.parse_args()
if args.report:assert Path(args.report).name==args.report
root=Path(__file__).resolve().parents[1];mode=('real' if args.real else 'arcade')+(f'-lookahead{args.lookahead}' if args.lookahead else '')+('-braked' if args.brake_bends else '')+(f'-min{args.min_length}' if args.min_length else '')+('-review' if args.review else '')+('-main' if args.main else '')+('-packaged' if args.app else '');log=root/f'work/lake-drive-{args.path}-{mode}.log'
capture=root/f'work/lake-capture-{args.path}-{mode}'/uuid.uuid4().hex
if args.app and args.review:
 bid=plistlib.loads((args.app.parent.parent/'Info.plist').read_bytes())['CFBundleIdentifier'];capture=Path.home()/'Library/Containers'/bid/'Data/Documents/LakeRouteReview'/uuid.uuid4().hex
...
if args.main:cmd[2]=cmd[2].replace('PiedmontLakePathGradingReview','PiedmontWorld')
if args.app:cmd=[str(args.app.resolve())]+cmd[2:]
if args.review:
...
text=log.read_text(errors='replace');rows=re.findall(r'BattleConnectorAudit: (\{[^\n]+\})',text);report=json.loads(rows[-1]) if rows else {'passed':False,'missing_report':True};report.update(exit_code=code,mode=mode,path=args.path,scope=('Packaged ' if args.app else 'Uncooked ')+('main' if args.main else 'candidate')+' map, two-direction keyboard traversal. Visual quality requires image review.');report['passed']=report['passed'] and code==0
...
report['runtime']='packaged' if args.app else 'editor-game'
report['map']='PiedmontWorld' if args.main else 'PiedmontLakePathGradingReview'
...
(root/'Tests/Results'/(args.report or f'2026-09-13-lake-drive-{args.path}-{mode}.json')).write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report))
