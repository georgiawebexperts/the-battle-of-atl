...
root=pathlib.Path(unreal.Paths.project_dir());out=root/'work/lake-path-shading-review';out.mkdir(parents=True,exist_ok=True)
...
views=[('lake-bank',[-3500,-2600,-100],[-6500,-4500,-250])]
images=[]
for name,position,target in [(n+'-'+label,p,t) for label in ['baseline','hidden-excluded','no-shadows','no-nanite'] for n,p,t in views]:
 if name.endswith('hidden-excluded'):
  hidden=[]
  for actor in ea.get_all_level_actors():
  ...
  (out/'hidden.json').write_text(json.dumps(hidden,indent=2))
 if name.endswith('no-shadows'):
  for actor in ea.get_all_level_actors():
   for primitive in actor.get_components_by_class(unreal.PrimitiveComponent):
    primitive.set_cast_shadow(False)
 if name.endswith('no-nanite'):
