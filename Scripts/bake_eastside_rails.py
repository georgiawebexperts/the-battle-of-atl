...
from arcade_trail import rail_offset_cm,TRAIL_WIDTH_ARCADE_CM,TRAIL_WIDTH_REALISTIC_CM
...
def bake(directory,width_cm,suffix):
 """Bake one guardrail set at one authored trail width.

 Rails sit RAIL_INSET_CM inside the pavement edge, so the narrow ribbon needs its
 own set: reusing the wide rails would leave them standing out in the grass.
 """
 out=root/'SourceAssets/Terrain'/directory
 ...
 for profile in h.profiles:
  ...
  for sign in [-1,1]:
   edge=[]
   for s in stations:
    p=np.array(line.interpolate(s).coords[0]);a=np.array(line.interpolate(s-1).coords[0]);b=np.array(line.interpolate(s+1).coords[0]);d=b-a;normal=np.array([-d[1],d[0]])/np.linalg.norm(d);q=p+normal*sign*rail_offset_cm(width_cm)
  ...
  name='EastsideBridge'+suffix+'_'+str(profile['osm_id'])+'_Rails';v=np.array(m['v']);lines=['# Authored arcade rail dimensions; OSM alignment, ODbL',f'o {name}']
 ...
 manifest['rail_width_game_cm']=width_cm;manifest['rail_offset_cm']=rail_offset_cm(width_cm)
 (out/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
 print('Baked',len(manifest['chunks']),'meshes at',width_cm,'cm,',manifest['triangle_count'],'triangles')

# The wide arcade rails keep the original file names so nothing else has to move;
# the narrow set is suffixed so both can live in the level at once.
bake('EastsideTrail',TRAIL_WIDTH_ARCADE_CM,'')
bake('EastsideTrailRealistic',TRAIL_WIDTH_REALISTIC_CM,'Realistic')
