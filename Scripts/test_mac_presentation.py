...
p=argparse.ArgumentParser();p.add_argument('--market-review',action='store_true');p.add_argument('--offscreen',action='store_true');p.add_argument('--width',type=int,choices=[1280,1920],default=1920);p.add_argument('--build',default='027');p.add_argument('--rig',action='store_true');p.add_argument('--locomotion',action='store_true');p.add_argument('--time-review',action='store_true');p.add_argument('--police-review',action='store_true');p.add_argument('--drone-review',action='store_true');p.add_argument('--bench-review',action='store_true');p.add_argument('--zombie-review',action='store_true');p.add_argument('--rifle-review',action='store_true');p.add_argument('--skate-review',action='store_true');p.add_argument('--horn-review',action='store_true');p.add_argument('--home-review',action='store_true');p.add_argument('--tutorial-review',action='store_true');args=p.parse_args();height=args.width*9//16
assert re.fullmatch(r'[0-9]{3}',args.build)
assert not (args.rig and args.locomotion)
out=root/'work'/f'build{args.build}-packaged-{args.width}{"-tutorial" if args.tutorial_review else ""}';out.mkdir(parents=True,exist_ok=True)
...
with (out/'render.log').open('w') as log:
 run=subprocess.run([str(app),'/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty=Easy?AutoStart=1','-windowed',*(['-RenderOffscreen'] if args.offscreen else []),*(['-BattleMarketReview'] if args.market_review else []),f'-ResX={args.width}',f'-ResY={height}','-ForceRes','-NoTextureStreaming','-unattended','-nosound','-BattleHUDReview',*(['-BattleRigReview'] if args.rig else []),*(['-BattleLocomotionReview'] if args.locomotion else []),*(['-BattleTimeReview'] if args.time_review else []),*(['-BattlePoliceReview'] if args.police_review else []),*(['-BattleDroneReview'] if args.drone_review else []),*(['-BattleBenchReview'] if args.bench_review else []),*(['-BattleZombieReview'] if args.zombie_review else []),*(['-BattleRifleReview'] if args.rifle_review else []),*(['-BattleSkateReview'] if args.skate_review else []),*(['-BattleHornReview'] if args.horn_review else []),*(['-BattleHomeReview'] if args.home_review else []),*(['-BattleTutorialReview'] if args.tutorial_review else []),'-ExecCmds=r.ScreenshotDelegate 0,t.IdleWhenNotForeground 0',f'-BattleHUDReviewDir={capture}','-stdout'],stdout=log,stderr=subprocess.STDOUT,timeout=180)
images=[]
...
text=(out/'render.log').read_text();assert ('LocoReview: pass=1' if args.locomotion else 'HUDReview: requested bike and foot captures') in text and run.returncode==0
if args.rig:
...
report={'market_review':args.market_review,'locomotion_review':args.locomotion,'captures_passed':True,'exit_code':run.returncode,'images':images,'visual_review':'pending','scope':('Native cooked Mac game, controlled walking/running/idle fixture; audio disabled; no physical input or FPS acceptance.' if args.locomotion else 'Native cooked Mac game, controlled bike-to-foot fixture; audio disabled; no physical input or FPS acceptance.')}
(root/'Tests/Results'/f'2026-09-11-build{args.build}-presentation-{args.width}{"-tutorial" if args.tutorial_review else ""}.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report))
