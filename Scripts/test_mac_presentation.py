...
with (out/'render.log').open('w') as log:
 run=subprocess.run([str(app),'/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty=Easy?AutoStart=1','-windowed',f'-ResX={args.width}',f'-ResY={height}','-ForceRes','-NoTextureStreaming','-unattended','-nosound','-BattleHUDReview',*(['-BattleRigReview'] if args.rig else []),*(['-BattleLocomotionReview'] if args.locomotion else []),*(['-BattleTimeReview'] if args.time_review else []),*(['-BattlePoliceReview'] if args.police_review else []),'-ExecCmds=r.ScreenshotDelegate 0',f'-BattleHUDReviewDir={capture}','-stdout'],stdout=log,stderr=subprocess.STDOUT,timeout=180)
images=[]
