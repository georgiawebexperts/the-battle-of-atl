...
expected={'SM_TenthStreet_'+name for name in ['Road','CycleTrack','Separator','Sidewalk','WhitePaint','YellowPaint']}
assert {a.get_actor_label() for a in actors}==expected and len(actors)==6
rows=[]
for actor in actors:
 comp=actor.static_mesh_component;mesh=comp.static_mesh
 expected_profile='NoCollision' if 'Paint' in actor.get_actor_label() else 'BlockAll'
 assert mesh and str(comp.get_collision_profile_name())==expected_profile
 ...
 rows.append({'label':actor.get_actor_label(),'asset':mesh.get_path_name(),'collision':expected_profile})
