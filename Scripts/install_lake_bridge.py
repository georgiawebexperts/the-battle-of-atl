...
p=pathlib.Path(unreal.Paths.project_dir());base=p/'SourceAssets/Terrain/LakeBridge';data=json.loads((base/'manifest.json').read_text());ea=unreal.get_editor_subsystem(unreal.EditorActorSubsystem);editor=unreal.get_editor_subsystem(unreal.StaticMeshEditorSubsystem);rows=[]
try:
 ...
 for c in data['chunks']:
  ...
  label='Lake crossing '+c['kind'];actor=existing.get(label) or ea.spawn_actor_from_class(unreal.StaticMeshActor,unreal.Vector());actor.set_actor_label(label);actor.set_folder_path('Piedmont/Bridges/Lake');actor.static_mesh_component.set_static_mesh(mesh);actor.static_mesh_component.set_collision_profile_name('BlockAll');actor.tags=[unreal.Name('RideBridge'),unreal.Name('RidePath')] if c['kind']!='Rails' else [unreal.Name('RideBarrier')]
 ...
 for actor in ea.get_all_level_actors():
  if isinstance(actor,unreal.PiedmontPathSpline):
   ...
   if way in ['102679938','146304988']:actor.set_centerline([unreal.Vector(*v) for v in data['centerline_cm' if way=='102679938' else 'spur_cm']])
...
(p/'Scripts/lake-bridge-install.json').write_text(json.dumps(report,indent=2))
