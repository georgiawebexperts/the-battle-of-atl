...
root=Path(__file__).resolve().parents[1];p=argparse.ArgumentParser();p.add_argument('--review',action='store_true');p.add_argument('--editor',action='store_true');p.add_argument('--detailed-rider',action='store_true');p.add_argument('--report');p.add_argument('--body',action='store_true');a=p.parse_args();mode='review' if a.review else 'audit'
...
if a.detailed_rider:
 ...
 d['passed']=bool(d['passed'] and d['detailed_arms_loaded'] and d['detailed_body_loaded'] and d['pose_transitions_completed'])
