...
with log.open('w') as f:
 r=subprocess.run(entry+['/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty=Easy?AutoStart=1','-game',('-RenderOffscreen' if a.review else '-nullrhi'),'-BattleSkipTutorial','-'+flag,'-RCWebControlDisable','-unattended','-stdout']+(['-BattleSpareBikeReviewDir='+str(capture),'-ResX=1280','-ResY=720','-windowed'] if a.review else [])+([] if a.kind=='music' else ['-nosound']),stdout=f,stderr=subprocess.STDOUT,timeout=90)
if capture!=out:
