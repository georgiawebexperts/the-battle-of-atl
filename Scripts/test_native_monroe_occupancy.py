...
rows=re.findall(r'MonroeOccupancy: (\{[^\n]+\})',log.read_text());r={'exit_code':run.returncode,'checks':json.loads(rows[-1]) if rows else None,'scope':'Two opposing cars held on green while the player stands in the crossing on foot, then resumed once they stepped clear. CanEnter() holds for a vehicle or a player-controlled pawn only, since 406c376 retired the pedestrian rule.'};r['passed']=run.returncode==0 and r['checks'] is not None and r['checks']['passed']
