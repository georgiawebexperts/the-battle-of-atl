...
names=['Idle/M_Relaxed_Stand_Idle_Loop','Walk/M_Relaxed_Walk_Loop_F','Run/M_Relaxed_Run_Loop_F','Idle/M_Neutral_Crouch_Idle_Loop','Crouch/M_Neutral_Crouch_Loop_F','Jump/M_Relaxed_Jump_Loop_Fall']
...
dest='/Game/BattleRetarget/Ellison/CityLocomotion';assert not unreal.EditorAssetLibrary.does_directory_exist(dest),'Do not overwrite previous exports'
...
(root/'Tests/Results/2026-09-13-detailed-locomotion-retarget.json').write_text(json.dumps({'source':source.get_path_name(),'target':target.get_path_name(),'retargeter':retargeter.get_path_name(),'clips':rows,'scope':'Retargeted copies only; not integrated or visually accepted.'},indent=2)+'\n')
