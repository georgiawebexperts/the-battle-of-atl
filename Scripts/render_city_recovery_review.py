...
root=pathlib.Path(unreal.Paths.project_dir());out=root/'work/city-recovery-review';out.mkdir(parents=True,exist_ok=True)
...
results=[]
variants=[]
for sex,prefix,outfit,face,hair in [('Male','m_tal_nrw','crewneck','m_001','Hair_S_AfroFade'),('Female','f_tal_nrw','scoopneck','f_001','Hair_S_Coil')]:
 for side in ['F','B','L','R']:variants.append((sex,prefix,outfit,face,hair,'/Game/BattleRetarget/City/'+sex+'/M_ragdoll_getup_stand_'+side))
...
(out/'manifest.json').write_text(json.dumps({'map_saved':False,'diagnostic_fill_light':{'intensity':40,'radius':1000},'explicit_sampled_pose_evaluation':True,'post_process_disabled_for_diagnostic':True,'hair_attached_to_head':True,'results':results,'scope':'Recovery poses only; not ragdoll or transition acceptance'},indent=2)+'\n')
print('BATTLE_RECOVERY_RENDER_COMPLETE')
