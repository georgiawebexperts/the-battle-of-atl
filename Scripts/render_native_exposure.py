...
report={'passed':len(runs)==4 and all(r['passed'] for r in runs),'runs':runs,'map_saved':False,'visual_review':False,'scope':'Native stationary player camera comparison; no movement through exposure transitions or release acceptance.'}
(root/'Tests/Results/2026-09-14-native-exposure.json').write_text(json.dumps(report,indent=2)+'\n')
