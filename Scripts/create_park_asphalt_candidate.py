"""Create a world-mapped asphalt candidate and apply only to the sleeper review map."""
import unreal,json,pathlib
root=pathlib.Path(unreal.Paths.project_dir());lib=unreal.MaterialEditingLibrary
path='/Game/PiedmontRide/Materials/M_ParkAsphaltWorld'
assert not unreal.EditorAssetLibrary.does_asset_exist(path),'Inspect existing candidate before rebuilding'
m=unreal.AssetToolsHelpers.get_asset_tools().create_asset('M_ParkAsphaltWorld','/Game/PiedmontRide/Materials',unreal.Material,unreal.MaterialFactoryNew())
def node(cls,**props):
...
color=mul(node(unreal.MaterialExpressionConstant3Vector,constant=unreal.LinearColor(.045,.048,.052)),shade)
...
(root/'Tests/Results/2026-09-12-asphalt-candidate.json').write_text(json.dumps({'material':path,'review_map_only':True,'actors':changed,'visual_accepted':False,'scope':'Procedural asphalt aggregate; no slab joints; no geometry changes'},indent=2)+'\n')
