...
p=argparse.ArgumentParser();p.add_argument('--build',default='040');a=p.parse_args();assert re.fullmatch(r'[0-9]{3}',a.build)
...
(root/f'Tests/Results/2026-09-11-build{a.build}-skatepark.json').write_text(json.dumps(data,indent=2)+'\n');print(json.dumps(data));raise SystemExit(0 if data['passed'] else 1)
