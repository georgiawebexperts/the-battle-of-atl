...
parser=argparse.ArgumentParser();parser.add_argument('--report',default='2026-09-11-build042-home-drive.json');args=parser.parse_args()
assert Path(args.report).name==args.report
app=root/'Saved/StagedBuilds/Mac/AuraPlayground.app/Contents/MacOS/AuraPlayground'
log=root/'work/mac-home-drive.log'
with log.open('w') as stream:
    run=subprocess.run([str(app),'/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty=Easy?AutoStart=1',
        '-ExecCmds=t.IdleWhenNotForeground 0','-nullrhi','-unattended','-nosound','-BattleHomeDriveAudit','-stdout'],stdout=stream,stderr=subprocess.STDOUT,timeout=600)
...
result['scope']='Krog southern exit along sourced Wylie Street to 98 Estoria patio, both directions using real W/A/D input and CharacterMovement; crowds disabled to isolate terrain/decks; appearance unverified.'
