...
import json, re, subprocess, plistlib, uuid, shutil
...
for variant in ['male','female']:
 ...
 out=root/'work/build044-city-review'/variant;out.mkdir(parents=True,exist_ok=True)
 ...
 markers=re.findall(r'CityLocoReview: ([^\n]+)',(out/'run.log').read_text())
 missing=[]
 for name in ['walk','walk-step','run','idle']:
...
(root/'Tests/Results/2026-09-11-build044-city-walkers.json').write_text(json.dumps(reports,indent=2)+'\n')
