...
p=argparse.ArgumentParser();p.add_argument("--bump",action="store_true");args=p.parse_args()
...
for variant in ['male','female']:
 ...
 out=root/('work/build044-city-bump-review' if args.bump else 'work/build044-city-review')/variant;out.mkdir(parents=True,exist_ok=True)
...
(root/('Tests/Results/2026-09-11-build044-city-bumps.json' if args.bump else 'Tests/Results/2026-09-11-build044-city-walkers.json')).write_text(json.dumps(reports,indent=2)+'\n')
