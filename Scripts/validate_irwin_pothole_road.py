...
path='/Game/BattleForTheA/Environment/IrwinTraffic';name='SM_Irwin_PotholeRoad'
task=unreal.AssetImportTask();task.filename=str(folder/'Irwin_PotholeRoad.obj');task.destination_path=path;task.destination_name=name
...
a=ea.spawn_actor_from_class(unreal.StaticMeshActor,unreal.Vector());a.set_actor_label('Irwin pothole road')
a.tags=[unreal.Name('IrwinPotholeRoad'),unreal.Name('RidePath')];a.set_folder_path('Eastside/IrwinLake')
...
if passed:assert unreal.EditorLoadingAndSavingUtils.save_map(unreal.get_editor_subsystem(unreal.UnrealEditorSubsystem).get_editor_world(),'/Game/PiedmontRide/Maps/PiedmontIrwinPotholeReview')
...
(root/'Tests/Results/2026-09-12-irwin-pothole-road-support.json').write_text(json.dumps(report,indent=2)+'\n')
