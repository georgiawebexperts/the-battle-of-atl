...
root=pathlib.Path(unreal.Paths.project_dir());folder=root/'SourceAssets/Terrain/LakePathGrading/Connections';ea=unreal.get_editor_subsystem(unreal.EditorActorSubsystem);dest='/Game/BattleForTheA/Environment/LakePathGrading/Connections';rows=[]
...
(root/'work/lake-graded-path-connections.json').write_text(json.dumps({'changes':rows},indent=2)+'\n')
