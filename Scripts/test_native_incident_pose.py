...
report={'exit_code':result.returncode,'checks':json.loads(rows[-1]) if rows else None,'scope':'Native held pose after 0.6s transition settling, horn and timed get-up completion, bike-impact and damage interruption; spawned fixture only, posed head visibility trace, empty standing-height trace and pelvis bike-sized Pawn-object sweep and blocked get-up wait; no whole-body envelope, scene placement, rarity or visual acceptance.'}
