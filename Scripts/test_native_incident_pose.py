...
report={'exit_code':result.returncode,'checks':json.loads(rows[-1]) if rows else None,'scope':'Native held pose after 0.6s transition settling, horn and timed get-up completion, bike-impact and damage interruption; spawned fixture only, no scene placement, rarity, visual or collision-envelope acceptance.'}
report['passed']=result.returncode==0 and bool(report['checks']) and report['checks']['passed']
(root/'Tests/Results/2026-09-12-native-incident-pose.json').write_text(json.dumps(report,indent=2)+'\n')
