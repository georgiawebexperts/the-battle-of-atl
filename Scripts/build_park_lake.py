...
try:
 ...
 if not land:
  ...
  land=unreal.PiedmontWorldTools.import_measured_landscape(str(data/'atlanta-height-lakebed.r16'),*meta['size'],unreal.Vector(*meta['unreal_location_cm']),unreal.Vector(*meta['unreal_scale']))
 ...
 ring=lake_data['outer_cm'];island=lake_data['island_cm'];cx=sum(v[0] for v in ring[:-1])/(len(ring)-1);cy=sum(v[1] for v in ring[:-1])/(len(ring)-1);z=lake_data['water_z_cm']
