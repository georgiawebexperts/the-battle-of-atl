...
p=argparse.ArgumentParser();p.add_argument('--packaged',action='store_true');p.add_argument('--report',default='2026-09-16-world-hill-momentum.json');a=p.parse_args()
...
with log.open('w') as stream:
 run=subprocess.run([*entry,'/Game/PiedmontRide/Maps/PiedmontWorld?Difficulty=Easy?AutoStart=1','-game','-nullrhi','-RCWebControlDisable','-BattleSkipTutorial','-BattleSteeringAudit','-BattleWorldHillAudit','-unattended','-nosound','-stdout'],stdout=stream,stderr=subprocess.STDOUT,timeout=90)
text=log.read_text();rows=re.findall(r'WorldHillAudit: (\{[^\n]+\})',text);samples=[json.loads(x) for x in re.findall(r'WorldHillSample: (\{[^\n]+\})',text)]
report=json.loads(rows[-1]) if rows else {'passed':False,'missing_report':True};report.update(samples=samples,exit_code=run.returncode,packaged=a.packaged,log=str(log),source='Installed mapped hillside beside 10th Street; source route OSM 1035389486 over active USGS terrain.',scope='Native current-world coasting at 900 cm/s for 1.5 seconds in Arcade and Real Bike Physics. Traffic and pedestrians disabled; subjective feel and full downhill route remain playtest checks.')
