...
void TickBattleRoadCrossingAudit(APlayerController* PC,float Dt){
 ...
 struct FState{TWeakObjectPtr<UWorld> World;TWeakObjectPtr<ABattleRoadCar> Car;TWeakObjectPtr<ABattleRoadCrossing> Gate;TWeakObjectPtr<AActor> Occupant;TWeakObjectPtr<AActor> Walker;float Clock=0,StoppedAt=0;int Phase=0;bool Done=false;};static FState S;
...
#define CHECK_CROSSING(C,R) if(!(C)){Finish(false,TEXT(R));return;}
 ...
 if(S.Phase==1&&S.Clock>7){
  ...
  S.Gate->bVehicleGreen=true;
  // A walker in the carriageway must NOT hold the traffic. Counting walkers as
  // occupants is what left the junction full of stationary cars.
  auto* Walker=PC->GetWorld()->SpawnActor<APiedmontPedestrian>();Walker->SetActorLocation(FVector(-18000,13520,420));S.Walker=Walker;
  CHECK_CROSSING(S.Gate->CanEnter(Car),"A walker in the crossing held the traffic");
  // A second vehicle in the box does hold this one.
  auto* Occupant=PC->GetWorld()->SpawnActor<ABattleRoadCar>();Occupant->SetActorLocation(FVector(-18000,13498,420));S.Occupant=Occupant;
  CHECK_CROSSING(!S.Gate->CanEnter(Car),"Green ignored a vehicle in the crossing");
  S.Phase=2;S.Clock=0;return;
 }
 if(S.Phase==2&&S.Clock>2){CHECK_CROSSING(FMath::Abs(Car->DistanceTravelled-S.StoppedAt)<1&&Car->Speed<1,"Entered occupied crossing");S.Occupant->Destroy();if(S.Walker.IsValid())S.Walker->Destroy();S.Phase=3;S.Clock=0;return;}
