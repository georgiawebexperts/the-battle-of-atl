...
void TickBattlePlayerCrashAudit(APlayerController* PC,float Dt){
 ...
 struct FState{TWeakObjectPtr<UWorld> World;TWeakObjectPtr<ABattleBike> Bike;TWeakObjectPtr<ABattleRoadCar> Car;float Clock=0,CrashTime=0,StartTime=0,Elapsed=0;int Stage=0,Wipeouts=0,Ammo=0,Cycles=0;bool Done=false,FallShot=false,FootShot=false,Death=false;};static FState S;
 ...
 if(S.Stage==0){
  ...
  B->RiderHealth=70;B->HurtCooldown=100;B->DamageGrace=100;S.Wipeouts=B->Ride->Wipeouts;S.Ammo=B->PistolAmmo;S.Death=FParse::Param(FCommandLine::Get(),TEXT("BattleCrashDeathInterrupt"));S.Stage=1;S.Clock=0;Key(EKeys::W,true);return;
 }
 ...
 if(S.Stage==4&&S.Clock-S.CrashTime>.5f){
  ...
  const bool Pass=PC->GetPawn()==B&&!B->bParked&&!B->bCrashActive&&!IsValid(B->PlayerCrash)&&B->Ride->IsMovingOnGround()&&B->PistolAmmo==S.Ammo&&B->RiderHealth==70&&B->Ride->Wipeouts==S.Wipeouts+1&&Crashes==0&&Fallen==0;
  ...
  Finish(Pass,S.Cycles?TEXT("Two keyboard collisions, recoveries and E remounts; no leftover crash actors"):TEXT("Keyboard collision, on-foot recovery and E remount"));
 }
