...
void ABattleZombie::DropWeapon(){
 ...
 if(auto* Drop=GetWorld()->SpawnActorDeferred<ABattleWeaponCrate>(ABattleWeaponCrate::StaticClass(),FTransform(Spot),nullptr,nullptr,ESpawnActorCollisionHandlingMethod::AlwaysSpawn)){Drop->WeaponSlot=1+FMath::RandHelper(3);Drop->FinishSpawning(FTransform(Spot));Drop->SetLifeSpan(60);DroppedWeapon=Drop;}
}
