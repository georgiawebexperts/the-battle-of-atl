...
void TickMarketImpactAudit(ABattleMarketClosure* Fence,float Dt){
 ...
 if(!Started){
  if(Junction&&OnFoot&&!Bike->Dismount()){Finish(false,TEXT("Initial practice dismount failed"));return;}
  Start=Junction?Fence->GetActorLocation()+FVector(0,640,0)-Heading.Vector()*1000:Fence->GetActorLocation()-FVector(1400,-Offset,0);FHitResult Floor;FCollisionQueryParams Q;Q.AddIgnoredActor(Bike);
