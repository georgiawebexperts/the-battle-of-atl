...
void TickMarketImpactAudit(ABattleMarketClosure* Fence,float Dt){
 ...
 auto Finish=[&](bool Pass,const TCHAR* Why){Key(EKeys::W,false);Key(EKeys::LeftShift,false);Key(EKeys::SpaceBar,false);UE_LOG(LogTemp,Display,TEXT("MarketImpactAudit: {\"passed\":%s,\"jump\":%s,\"on_foot\":%s,\"reason\":\"%s\",\"closest_cm\":%.2f,\"peak_cm\":%.2f,\"airborne\":%s}"),Pass?TEXT("true"):TEXT("false"),Jump?TEXT("true"):TEXT("false"),OnFoot?TEXT("true"):TEXT("false"),Why,Closest,Peak,Air?TEXT("true"):TEXT("false"));FPlatformMisc::RequestExit(false);};
 if(!Started){
  ...
  Start.Z=Floor.ImpactPoint.Z+98;Bike->SetActorLocationAndRotation(Start,FRotator::ZeroRotator,false,nullptr,ETeleportType::TeleportPhysics);Bike->Ride->StopMovementImmediately();Bike->Ride->Speed=0;Bike->Ride->Gear=4;Bike->Ride->bForceNextFloorCheck=true;Bike->DamageGrace=100;PC->SetControlRotation(FRotator::ZeroRotator);if(OnFoot){if(!Bike->Dismount()){Finish(false,TEXT("Dismount failed"));return;}Start=PC->GetPawn()->GetActorLocation();Key(EKeys::LeftShift,true);}Started=true;Key(EKeys::W,true);
 }
 Clock+=Dt;const FVector P=PC->GetPawn()->GetActorLocation();const float Gap=Fence->GetActorLocation().X-P.X;Closest=FMath::Min(Closest,Gap);Peak=FMath::Max(Peak,float(P.Z-Start.Z));if(auto* Foot=Cast<ABattleRider>(PC->GetPawn()))Air|=Foot->GetCharacterMovement()->IsFalling();else Air|=Bike->Ride->IsFalling();
