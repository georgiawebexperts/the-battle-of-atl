...
void TickBattleCarReview(APlayerController* PC,float Dt){
 ...
 struct FState{TWeakObjectPtr<UWorld> World;TWeakObjectPtr<ABattleRoadCar> Driving;TWeakObjectPtr<ACameraActor> Camera;float Clock=0;int Phase=0;};static FState S;
 ...
 if(S.Phase==0){
  ...
  if(FParse::Param(FCommandLine::Get(),TEXT("BattleDrivingCarReview"))){
   ...
   if(!Driving->StartRoute()){PC->ConsoleCommand(TEXT("quit"));S.Phase=3;return;}S.Driving=Driving;
  }else{
