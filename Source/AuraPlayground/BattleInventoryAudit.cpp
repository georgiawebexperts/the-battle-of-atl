...
#define CHECK_INVENTORY(C,R) if(!(C)){Finish(false,TEXT(R));return;}
 ...
 else if(InventoryPhase==7){Aim();if(InventoryClock>.4f){CHECK_INVENTORY(Person->CurrentWeapon==2&&Person->Fire(),"3 key/SMG fire failed");InventoryShots++;InventoryPhase=8;InventoryClock=0;}}
