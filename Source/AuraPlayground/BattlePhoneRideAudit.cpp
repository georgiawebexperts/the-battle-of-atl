...
void TickBattlePhoneRideAudit(APlayerController* PC,float Dt){
 ...
 struct FState{TWeakObjectPtr<UWorld> World;TArray<FVector> Points;FVector Previous;int Next=1,Stage=0;float Clock=0,Distance=0,Still=0,Progress=0;bool Started=false,Done=false;};static FState S;
 ...
 if(!B||!M||!M->Quest){End(false,TEXT("Missing mounted player or quest"));return;}auto* Q=M->Quest.Get();const bool Full=FParse::Param(FCommandLine::Get(),TEXT("BattleFullRide"));
 if(!S.Started){
 ...
 if(Q->bCollected&&!Full){End(true,TEXT("Rode from park start and collected phone through proximity; no teleport"));return;}
 ...
 if(Full&&Stage!=S.Stage){
  ...
  S.Stage=Stage;S.Points=Q->RoutePoints;S.Next=1;S.Still=0;
 ...
 if(S.Clock>(Full?800:150)||S.Still>12){UE_LOG(LogTemp,Display,TEXT("PhoneRideFailure: position=%s target=%s speed=%.2f yaw=%.2f"),*P.ToString(),*S.Points[S.Next].ToString(),B->Ride->Speed,B->GetActorRotation().Yaw);End(false,TEXT("Guided input stalled or timed out; inspect controller and world"));return;}
