...
void TickBattleKrogRiderAudit(APlayerController* PC,float Dt){
 ...
 auto Finish=[&](bool Passed,const TCHAR* Reason){PC->FlushPressedKeys();S.Done=true;UE_LOG(LogTemp,Display,TEXT("KrogRiderAudit: {\"passed\":%s,\"reason\":\"%s\",\"travel_cm\":%.2f,\"max_error_cm\":%.2f,\"held_seconds\":%.2f,\"ground_samples\":%d,\"paved_samples\":%d}"),Passed?TEXT("true"):TEXT("false"),Reason,S.Travel,S.MaxError,S.Hold,S.Ground,S.Paved);PC->ConsoleCommand(TEXT("quit"));};
 if(!Bike||S.Clock>50){Finish(false,TEXT("Rider missing or timed out"));return;}
 if(S.Phase==0){
 ...
 if(S.Phase==2){
  ...
  if(FVector::Dist2D(P,Points[Count-1])<90){PC->FlushPressedKeys();S.Phase=3;return;}
