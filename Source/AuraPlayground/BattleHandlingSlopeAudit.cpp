...
void TickBattleHandlingSlopeAudit(APlayerController* PC,float Dt){
 ...
 if(!S.Started){
  ...
  Move->StopMovementImmediately();Move->Speed=Move->ReverseSpeed=0;Move->Recovery=0;Move->SmoothedSteer=0;Move->bRealHandling=true;Move->Gear=5;Move->SlideRemaining=Move->BoostRemaining=0;
 ...
 if(!S.Ready){if(S.Clock<.3f)return;if(!Move->IsMovingOnGround()){Finish(false,TEXT("Did not settle on fixture"));return;}
  Move->Speed=S.Phase==7?0:S.Phase==3||S.Phase==4?1600:600;Move->Velocity=Bike->GetActorForwardVector()*Move->Speed;S.StartYaw=Bike->GetActorRotation().Yaw;S.StartPosition=Bike->GetActorLocation();
 ...
 const bool Pass=S.Phase==0?(Move->Speed<500&&Move->Speed>300):S.Phase==1?(Move->Speed>700&&Move->Speed<900):S.Phase==2?Move->Speed<10:S.Phase==3?(S.MaxLateral<=685&&Yaw>10):S.Phase==4?(S.MaxLateral<=355&&Yaw>5):S.Phase==5?(Move->IsFalling()&&FMath::Abs(Move->Speed-600)<1&&Yaw<.01f):S.Phase==8?(Move->Speed<1&&Move->ReverseSpeed>100&&FVector::DotProduct(Bike->GetVelocity(),Bike->GetActorForwardVector())<-100):(Move->Speed<1&&Move->ReverseSpeed<1&&Bike->GetVelocity().Size2D()<10&&(S.Phase!=7||FVector::Dist2D(S.StartPosition,Bike->GetActorLocation())<5));
 ...
 if(S.Phase==8){Finish(true,TEXT("Uphill downhill road/grass grip, airborne behavior, steep-grass braking/holding/reverse passed"));return;}++S.Phase;S.Started=false;
#endif
}
