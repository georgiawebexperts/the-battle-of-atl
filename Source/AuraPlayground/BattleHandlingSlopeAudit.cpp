...
void TickBattleHandlingSlopeAudit(APlayerController* PC,float Dt){
 ...
 struct FState{TWeakObjectPtr<UWorld> World;TWeakObjectPtr<AStaticMeshActor> Floor;int Phase=0,Samples=0,Ground=0;float Clock=0,MaxLateral=0,StartYaw=0;bool Started=false,Ready=false,Done=false;};static FState S;
 ...
 auto Finish=[&](bool Pass,const TCHAR* Why){for(FKey K:{EKeys::W,EKeys::D,EKeys::SpaceBar})Key(K,false);S.Done=true;UE_LOG(LogTemp,Display,TEXT("HandlingSlopeAudit: {\"passed\":%s,\"reason\":\"%s\",\"phase\":%d}"),Pass?TEXT("true"):TEXT("false"),Why,S.Phase);PC->ConsoleCommand(TEXT("quit"));};
 if(!S.Started){
  for(FKey K:{EKeys::W,EKeys::D,EKeys::SpaceBar})Key(K,false);
  ...
  auto* Floor=S.Floor.Get();Floor->Tags.Empty();Floor->Tags.Add(S.Phase==4?TEXT("RideGrass"):TEXT("RidePath"));Floor->SetActorLocationAndRotation(FVector(0,0,30000),FRotator(S.Phase<2?10:0,0,0));
  ...
  Move->StopMovementImmediately();Move->Speed=0;Move->Recovery=0;Move->SmoothedSteer=0;Move->bRealHandling=true;Move->Gear=5;Move->SlideRemaining=Move->BoostRemaining=0;
  Bike->SetActorLocationAndRotation(Hit.ImpactPoint+FVector(0,0,98),FRotator(0,S.Phase==1?180:0,0),false,nullptr,ETeleportType::TeleportPhysics);Move->SetMovementMode(MOVE_Walking);Move->bForceNextFloorCheck=true;
 ...
 if(!S.Ready){if(S.Clock<.3f)return;if(!Move->IsMovingOnGround()){Finish(false,TEXT("Did not settle on fixture"));return;}
  Move->Speed=S.Phase==3||S.Phase==4?1600:600;Move->Velocity=Bike->GetActorForwardVector()*Move->Speed;S.StartYaw=Bike->GetActorRotation().Yaw;
  if(S.Phase==2)Key(EKeys::SpaceBar,true);if(S.Phase==3||S.Phase==4)Key(EKeys::D,true);
 ...
 if(S.Clock<(S.Phase==5?.2f:1.f))return;
 const float Yaw=FMath::Abs(FMath::FindDeltaAngleDegrees(S.StartYaw,Bike->GetActorRotation().Yaw));
 const bool Pass=S.Phase==0?(Move->Speed<500&&Move->Speed>300):S.Phase==1?(Move->Speed>700&&Move->Speed<900):S.Phase==2?Move->Speed<10:S.Phase==3?(S.MaxLateral<=685&&Yaw>10):S.Phase==4?(S.MaxLateral<=355&&Yaw>5):(Move->IsFalling()&&FMath::Abs(Move->Speed-600)<1&&Yaw<.01f);
 ...
 if(S.Phase==5){Finish(true,TEXT("Uphill downhill braking road grass and airborne fixtures passed"));return;}++S.Phase;S.Started=false;
#endif
}
