...
ABattleHornPickup::ABattleHornPickup(){
 ...
 for(int I=0;I<2;I++){auto* Label=CreateDefaultSubobject<UTextRenderComponent>(*FString::Printf(TEXT("HornLabel%d"),I));Label->SetupAttachment(RootComponent);Label->SetRelativeLocation(FVector(I?-2:2,0,43));Label->SetRelativeRotation(FRotator(0,I?180:0,0));Label->SetWorldSize(18);Label->SetHorizontalAlignment(EHTA_Center);Label->SetText(FText::FromString(TEXT("HORN +3")));Label->SetTextRenderColor(FColor(255,211,139));Label->SetTextMaterial(TextMat.Object);Label->SetCollisionEnabled(ECollisionEnabled::NoCollision);Label->SetCanEverAffectNavigation(false);}
}
