...
void ABattleWeaponCrate::BeginPlay(){
 ...
 for(int32 I=0;I<2;I++){auto* Text=NewObject<UTextRenderComponent>(this);Text->SetupAttachment(RootComponent);Text->SetRelativeLocation(FVector(I?-34:34,0,20));Text->SetRelativeRotation(FRotator(0,I?180:0,0));Text->SetText(FText::FromString(WeaponSlot==0?TEXT("PISTOL AMMO +10"):BattleWeapons::Name(WeaponSlot)));Text->SetWorldSize(10);Text->SetHorizontalAlignment(EHTA_Center);Text->SetTextRenderColor(FColor::Cyan);Text->SetCollisionEnabled(ECollisionEnabled::NoCollision);Text->SetCanEverAffectNavigation(false);Text->RegisterComponent();}
}
