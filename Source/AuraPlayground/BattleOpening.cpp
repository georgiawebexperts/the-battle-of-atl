...
namespace {
struct FOpeningRiderHold {TWeakObjectPtr<ABattleBike> Bike;bool ActorTick,MoveTick;};
TMap<TWeakObjectPtr<ABattleMacController>,FOpeningRiderHold> OpeningRiderHolds;
void ReleaseOpeningRider(TWeakObjectPtr<ABattleMacController> Controller){
 if(auto* Hold=OpeningRiderHolds.Find(Controller)){
  if(Hold->Bike.IsValid()){Hold->Bike->SetActorTickEnabled(Hold->ActorTick);Hold->Bike->Ride->SetComponentTickEnabled(Hold->MoveTick);}
  OpeningRiderHolds.Remove(Controller);
 }
}
void OpeningShot(float Seconds,FVector Home,FVector& Eye,FVector& Target){
 if(Seconds<8.f){
  const float T=FMath::SmoothStep(0.f,1.f,FMath::Clamp(Seconds/8.f,0.f,1.f));
  Eye=FMath::Lerp(FVector(1800,-6500,5000),FVector(700,-5400,4400),T);
  Target=FVector(-3500,0,-330);
 }else{
  const float T=FMath::SmoothStep(0.f,1.f,FMath::Clamp((Seconds-8.f)/4.f,0.f,1.f));
  Eye=Home+FMath::Lerp(FVector(-450,-650,250),FVector(350,-550,180),T);Target=Home+FVector(0,0,65);
 }
}
}
...
void ABattleMacController::BeginOpening(){
 ...
 FVector Eye,Target;OpeningShot(0,OpeningRiderLocation,Eye,Target);
 ...
 // Water tiles need live world updates as the aerial camera moves.
 // Keep the park alive, but hold the rider still throughout the intro.
 const bool OldActorTick=Bike->IsActorTickEnabled(),OldMoveTick=Bike->Ride->IsComponentTickEnabled();
 OpeningRiderHolds.Add(this,{Bike,OldActorTick,OldMoveTick});
 Bike->SetActorTickEnabled(false);Bike->Ride->SetComponentTickEnabled(false);
 FlushPressedKeys();SetIgnoreMoveInput(true);SetIgnoreLookInput(true);bShowMouseCursor=true;SetInputMode(FInputModeGameAndUI());if(GetHUD())GetHUD()->bShowHUD=false;
 ...
 Menu=SNew(SScaleBox).Stretch(EStretch::ScaleToFit)[SNew(SBox).WidthOverride(1280).HeightOverride(720)[SNew(SOverlay)
  +SOverlay::Slot().VAlign(VAlign_Top)[SNew(SBorder).BorderImage(FCoreStyle::Get().GetBrush("WhiteBrush")).BorderBackgroundColor(Ink).Padding(FMargin(46,24))[SNew(SVerticalBox)
   ...
   +SVerticalBox::Slot().AutoHeight().Padding(0,7,0,2).HAlign(HAlign_Left)[SNew(SBox).WidthOverride(142)[SNew(SBorder).BorderImage(FCoreStyle::Get().GetBrush("WhiteBrush")).BorderBackgroundColor(Gold).Padding(FMargin(12,6))[SNew(STextBlock).Text(FText::FromString(BattleBuild::Label)).Font(FCoreStyle::GetDefaultFontStyle("Bold",18)).ColorAndOpacity(FLinearColor(.025,.035,.045))]]]
   +SVerticalBox::Slot().AutoHeight().Padding(0,8)[SNew(STextBlock).Text_Lambda([this](){return FText::FromString(FPlatformTime::Seconds()-OpeningStart<8?TEXT("ATLANTA  /  PIEDMONT PARK"):TEXT("MIDTOWN  /  13TH STREET"));}).Font(FCoreStyle::GetDefaultFontStyle("Regular",18)).ColorAndOpacity(Gold)]]]
 ...
 UE_LOG(LogTemp,Display,TEXT("BattleOpening: started; rider held; timer=%.0f"),OpeningTimer);
 FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda([Weak=TWeakObjectPtr<ABattleMacController>(this)](float){
  if(!Weak.IsValid()||!Weak->bOpeningActive){ReleaseOpeningRider(Weak);return false;}
  ...
  FVector Eye,Target;OpeningShot(float(T),C->OpeningRiderLocation,Eye,Target);
#if !UE_BUILD_SHIPPING
...
void ABattleMacController::FinishOpening(){
 ...
 if(FParse::Param(FCommandLine::Get(),TEXT("BattleOpeningAudit"))||FParse::Param(FCommandLine::Get(),TEXT("BattleOpeningReview"))){
  auto* Bike=Cast<ABattleBike>(GetPawn());const bool RiderReleased=Bike&&Bike->IsActorTickEnabled()&&Bike->Ride->IsComponentTickEnabled();
  const bool Skip=FParse::Param(FCommandLine::Get(),TEXT("BattleOpeningSkip"));BeginOpening();const bool Pass=RiderReleased&&!bOpeningActive&&bOpeningSeen&&CameraTravel>(Skip?10:500)&&Protected&&!IsPaused()&&!Menu.IsValid()&&GetViewTarget()==GetPawn()&&(Skip?Duration<4:Duration>=12);
