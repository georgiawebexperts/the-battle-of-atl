...
void ABattleMacController::TickJumpAudit(float Dt){
 ...
 if(!FParse::Param(FCommandLine::Get(),TEXT("BattleJumpSideReview")))return;
  ...
  const FVector Eye=Close?Bike->Visual->GetComponentTransform().TransformPosition(FVector(0,-160,80)):Bike->GetActorTransform().TransformPosition(FVector(25,-450,170)),Focus=Close?Bike->Visual->GetComponentTransform().TransformPosition(FVector(-5,0,36)):Bike->GetActorLocation()+FVector(0,0,45);
