...
#define BCHECK(C,R) if(!(C)){Finish(false,TEXT(R));return;}
 ...
 auto CheckWheels=[&](AActor* Actor){int Counted=0;bool Good=true;
   TInlineComponentArray<UStaticMeshComponent*> Parts(Actor);
   for(auto* Part:Parts)if(Part->GetStaticMesh()&&Part->GetStaticMesh()->GetName()==TEXT("SM_BikeWheel")){
    const FBox Bounds=Part->Bounds.GetBox();const FVector Bottom(Bounds.GetCenter().X,Bounds.GetCenter().Y,Bounds.Min.Z);
    FHitResult Ground;FCollisionQueryParams Q(SCENE_QUERY_STAT(SpareWheelGroundAudit),true,Actor);Q.AddIgnoredActor(Bike);
    const bool Hit=PC->GetWorld()->LineTraceSingleByChannel(Ground,Bottom+FVector(0,0,50),Bottom-FVector(0,0,300),ECC_Visibility,Q);
    UE_LOG(LogTemp,Display,TEXT("SpareWheelGroundAudit: actor=%s wheel=%s gap_cm=%.3f bottom=%s"),*Actor->GetName(),*Part->GetName(),Hit?Bottom.Z-Ground.ImpactPoint.Z:999.f,*Bottom.ToString());
    Counted++;Good&=Hit&&FMath::Abs(Bottom.Z-Ground.ImpactPoint.Z)<1.f;
   }
  return Good&&Counted==2;
 };
 ...
 if(Phase==0){
  ...
  // Isolate transfer conservation from collecting a nearby station's supplies.
  // Keep their meshes and collision so the real-world approach remains intact.
  for(TActorIterator<ABattleWeaponCrate> It(PC->GetWorld());It;++It)It->SetActorTickEnabled(false);
  for(TActorIterator<ABattleColaPickup> It(PC->GetWorld());It;++It)It->SetActorTickEnabled(false);
  for(TActorIterator<ABattleHornPickup> It(PC->GetWorld());It;++It)It->SetActorTickEnabled(false);
