...
#define CHECK_STEERING(C,R) if(!(C)){Finish(false,TEXT(R));return;}
 ...
 }else if(SteeringStage==1&&SteeringClock>.65f){CHECK_STEERING(Bike->Ride->Speed>250&&Bike->Ride->Gear==1&&FVector::Dist2D(SteeringStart,Bike->GetActorLocation())>70,"Up did not pedal or changed gear");Key(EKeys::Up,false);Key(EKeys::Down,true);Next();}
 ...
 else if(SteeringStage==9&&SteeringClock>.35f){CHECK_STEERING(Bike->Ride->SmoothedSteer<-.8f&&FMath::Abs(FMath::FindDeltaAngleDegrees(SteeringYaw,Bike->GetActorRotation().Yaw))<.01f,"Stationary bike pivoted in place");Key(EKeys::Left,false);Finish(true,TEXT("Actual arrow/WASD pedal/brake, eased steering reversal and lean, Q/R gears, remount reset, stationary heading and 30/120Hz response pass"));}
