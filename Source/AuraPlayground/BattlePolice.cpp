...
ABattlePolice::ABattlePolice(){
 ...
 TaserWarning=CreateDefaultSubobject<UTextRenderComponent>(TEXT("TaserWarning"));
 TaserWarning->SetupAttachment(GetCapsuleComponent());TaserWarning->SetRelativeLocation(FVector(0,0,125));
 TaserWarning->SetText(FText::FromString(TEXT("STOP! APD\nTASER — MOVE TO COVER")));
 TaserWarning->SetWorldSize(22);TaserWarning->SetHorizontalAlignment(EHTA_Center);
 TaserWarning->SetTextRenderColor(FColor(255,220,60));TaserWarning->SetCollisionEnabled(ECollisionEnabled::NoCollision);
 TaserWarning->SetVisibility(false);
...
bool ABattlePolice::FireTaser(){
 ...
 TaserShots++;Cooldown=15;bWarning=false;TaserWarning->SetVisibility(false);return Hit;
}
void ABattlePolice::Tick(float Dt){
 ...
 if(bWarning){if(AI)AI->StopMovement();SetActorRotation(FRotator(0,(Target->GetActorLocation()-GetActorLocation()).Rotation().Yaw,0));
  ...
  // Face the actual camera so the warning remains readable on foot and on the bike.
  if(auto* PC=UGameplayStatics::GetPlayerController(this,0)){FVector Eye;FRotator View;PC->GetPlayerViewPoint(Eye,View);TaserWarning->SetWorldRotation((Eye-TaserWarning->GetComponentLocation()).Rotation());}
  TaserWarning->SetVisibility(true);WarningRemaining-=Dt;if(WarningRemaining<=0)FireTaser();return;}
...
float ABattlePolice::TakeDamage(float Amount,const FDamageEvent& Event,AController* Instigator,AActor* Causer){
 ...
 bWarning=false;TaserWarning->SetVisibility(false);Cooldown=FMath::Max(Cooldown,.5f);
