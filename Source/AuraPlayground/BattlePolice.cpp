...
ABattlePolice::ABattlePolice(){
 ...
 for(int32 Side:{-1,1}){auto* Label=CreateDefaultSubobject<UTextRenderComponent>(Side>0?TEXT("PoliceFront"):TEXT("PoliceBack"));Label->SetupAttachment(GetCapsuleComponent());Label->SetRelativeLocation(FVector(Side*22,0,44));Label->SetRelativeRotation(FRotator(0,Side>0?0:180,0));Label->SetText(FText::FromString(TEXT("POLICE")));Label->SetWorldSize(18);Label->SetHorizontalAlignment(EHTA_Center);Label->SetTextRenderColor(FColor::White);Label->SetCollisionEnabled(ECollisionEnabled::NoCollision);}
