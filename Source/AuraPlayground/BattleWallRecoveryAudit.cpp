...
#define WCHECK(C,R) if(!(C)){Finish(false,TEXT(R));return;}
 ...
 if(S.Phase==0){
  ...
  B->SetActorLocationAndRotation(FVector(0,0,30150),FRotator::ZeroRotator,false,nullptr,ETeleportType::TeleportPhysics);M->SetMovementMode(MOVE_Walking);M->bForceNextFloorCheck=true;Next();
 ...
 else if(S.Phase==7&&S.Clock>1){WCHECK(M->Speed>150&&M->ReverseSpeed<1&&FVector::Dist2D(S.Start,B->GetActorLocation())>80,"Could not ride out after reversing");Finish(true,TEXT("Solid wall contact, reverse escape, reverse steering, brake-only stop and forward departure pass"));}
