...
void TickBattleBenchFireReview(APlayerController* PC,float Dt){
 ...
 if(S.Phase==0){
  for(TActorIterator<ABattleParkFurniture> It(PC->GetWorld());It;++It)if(It->Benches.Num()){
   ...
   if(FParse::Param(FCommandLine::Get(),TEXT("BattleBenchReachReview"))){
    ...
    UE_LOG(LogTemp,Display,TEXT("BenchReachReview: started=%d"),Visitor->BeginBenchReach());
 ...
 const float CaptureTimes[]={.12f,bReach?2.2f:2.f,5.85f};
