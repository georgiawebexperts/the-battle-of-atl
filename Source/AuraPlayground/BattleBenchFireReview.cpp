...
void TickBattleBenchFireReview(APlayerController* PC,float Dt){
 ...
 if(S.Phase>=1&&S.Phase<=3&&S.Clock>S.Phase){
 ...
 if(S.Clock>5&&S.Phase==4){S.Done=true;UE_LOG(LogTemp,Display,TEXT("BenchFireReview: captured=3 fire=%d"),S.Fire.IsValid());PC->ConsoleCommand(TEXT("quit"));}
