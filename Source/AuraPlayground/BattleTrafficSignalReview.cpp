...
void TickBattleTrafficSignalReview(APlayerController* PC,float Dt){
 ...
 if(S.Phase==0){
  FHitResult Hit;FCollisionQueryParams Q(SCENE_QUERY_STAT(TrafficSignalReview),true);if(!PC->GetWorld()->LineTraceSingleByChannel(Hit,FVector(-18500,13900,3000),FVector(-18500,13900,-3000),ECC_Visibility,Q)){S.Phase=5;PC->ConsoleCommand(TEXT("quit"));return;}
 ...
 if(S.Phase==2&&S.Clock>3){S.Gate->bVehicleGreen=true;S.Phase=3;}
 if(S.Phase==3&&S.Clock>4){FScreenshotRequest::RequestScreenshot(Folder/TEXT("signal-green.png"),false,false);S.Phase=4;}
 if(S.Phase==4&&S.Clock>5){S.Phase=5;UE_LOG(LogTemp,Display,TEXT("TrafficSignalReview: complete"));PC->ConsoleCommand(TEXT("quit"));}
#endif
}
