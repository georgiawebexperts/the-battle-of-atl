...
#define GUN_CHECK(C,R) if(!(C)){End(false,TEXT(R));return;}
 ...
 if(Phase==0){
  ...
  GUN_CHECK(Gun->TryAim(B),"Could not aim after cover removed");B->SetActorLocation(Origin+FVector(0,400,0),false,nullptr,ETeleportType::TeleportPhysics);if(!ReviewDir.IsEmpty()){const bool Grip=FParse::Param(FCommandLine::Get(),TEXT("GunmanGripReview"));if(Grip)if(auto* HUD=UGameplayStatics::GetPlayerController(W,0)->GetHUD())HUD->bShowHUD=false;const FVector Focus=Gun->GetActorLocation()+FVector(0,0,Grip?20:25);const FVector Eye=Gun->GetActorLocation()+(Grip?FVector(-130,-100,80):FVector(-450,-280,130));auto* Cam=W->SpawnActor<ACameraActor>(Eye,(Focus-Eye).Rotation());UGameplayStatics::GetPlayerController(W,0)->SetViewTarget(Cam);}Phase=2;Clock=0;
