...
void TickBattleIncidentPoseAudit(APlayerController* PC,float Dt){
 ...
 static FVector Pelvis;static bool Started=false,Held=false,Recovered=false;static float Drift=0;static bool Impact=false,Damage=false,TimedStarted=false;
 ...
 if(Stage==20&&Age>1){Drift=FVector::Dist(Pelvis,P->Body->GetBoneLocation(TEXT("pelvis")));Held=P->bIncidentPosing&&Drift<1.f;P->HearHorn(PC->GetPawn());Stage=3;Age=0;return;}
 ...
 if(Stage==4&&Age>Clip->GetPlayLength()+2){
  ...
  UE_LOG(LogTemp,Display,TEXT("BattleIncidentPoseAudit: {\"passed\":%s,\"started\":%s,\"held\":%s,\"pelvis_drift_cm\":%.3f,\"horn_recovered\":%s,\"bike_interrupt\":%s,\"damage_interrupt\":%s,\"timed_recovered\":%s}"),Started&&Held&&Recovered&&Impact&&Damage&&Timed?TEXT("true"):TEXT("false"),Started?TEXT("true"):TEXT("false"),Held?TEXT("true"):TEXT("false"),Drift,Recovered?TEXT("true"):TEXT("false"),Impact?TEXT("true"):TEXT("false"),Damage?TEXT("true"):TEXT("false"),Timed?TEXT("true"):TEXT("false"));
