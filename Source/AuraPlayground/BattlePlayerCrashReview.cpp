...
void TickBattlePlayerCrashReview(APlayerController* PC,float Dt){
 ...
 if(!S.Body.IsValid()){
  ...
  if(FParse::Param(FCommandLine::Get(),TEXT("BattleCrashSolidSupport"))){AActor* Support=PC->GetWorld()->SpawnActor<AActor>();auto* Box=NewObject<UBoxComponent>(Support);Support->AddInstanceComponent(Box);Support->SetRootComponent(Box);Box->SetBoxExtent(FVector(1000,1000,10));Box->SetWorldLocation(Hit.ImpactPoint-FVector(0,0,10));Box->SetCollisionProfileName(TEXT("BlockAll"));Box->RegisterComponent();}
  ...
  auto* Mesh=Cast<USkeletalMesh>(Bike->Rider->GetSkinnedAsset());auto* Physics=LoadObject<UPhysicsAsset>(nullptr,TEXT("/Game/BattleForTheA/Rider/Physics/PA_EllisonCrashCandidateV5.PA_EllisonCrashCandidateV5"));if(!Mesh||!Physics){S.Done=true;PC->ConsoleCommand(TEXT("quit"));return;}
 ...
 if(S.Clock>4.5f){
  ...
  float MinSkin=TNumericLimits<float>::Max();int32 Below=0;const auto& LOD=S.Body->GetSkeletalMeshAsset()->GetResourceForRendering()->LODRenderData[0];auto* Weights=Visible->GetSkinWeightBuffer(0);
  TMap<FName,int32> PenetratingBones;
  for(uint32 I=0;Weights&&I<LOD.GetNumVertices();I++){const FVector Point=Visible->GetComponentTransform().TransformPosition(FVector(USkinnedMeshComponent::GetSkinnedVertexPosition(Visible,I,LOD,*Weights)));FHitResult Surface;if(PC->GetWorld()->LineTraceSingleByChannel(Surface,Point+FVector(0,0,100),Point-FVector(0,0,200),ECC_WorldStatic,Q)){MinSkin=FMath::Min(MinSkin,float(Point.Z-Surface.ImpactPoint.Z));if(Point.Z<Surface.ImpactPoint.Z-1){Below++;for(const auto& Section:LOD.RenderSections)if(I>=Section.BaseVertexIndex&&I<Section.BaseVertexIndex+Section.NumVertices){uint32 Influence=0;uint16 Weight=0;for(uint32 J=0;J<Weights->GetMaxBoneInfluences();J++)if(Weights->GetBoneWeight(I,J)>Weight){Weight=Weights->GetBoneWeight(I,J);Influence=J;}const int32 LocalBone=Weights->GetBoneIndex(I,Influence);if(Section.BoneMap.IsValidIndex(LocalBone))PenetratingBones.FindOrAdd(S.Body->GetSkeletalMeshAsset()->GetRefSkeleton().GetBoneName(Section.BoneMap[LocalBone]))++;break;}}}}
  for(const auto& Row:PenetratingBones)UE_LOG(LogTemp,Display,TEXT("CrashSkinBone: %s vertices=%d"),*Row.Key.ToString(),Row.Value);
  ...
  UE_LOG(LogTemp,Display,TEXT("PlayerCrashReview: {\"passed\":%s,\"hip_drop_cm\":%.3f,\"max_limb_span_cm\":%.3f,\"hip_floor_cm\":%.3f,\"final_speed\":%.3f}"),Pass?TEXT("true"):TEXT("false"),Drop,S.MaxSpan,Clearance,Speed);if(FParse::Param(FCommandLine::Get(),TEXT("BattlePlayerRecoveryBlend"))&&Pass&&S.Blend.Begin(Bike,S.Body.Get(),S.Display.Get()))S.Recovering=true;else{S.Done=true;PC->ConsoleCommand(TEXT("quit"));}
 }
#endif
}
